...
class SimilaritySearchEngine:
    def __init__(self, id_embedding_dict: dict):
        """
        Initialize with a dictionary of {item_id: embedding_vector}
        :param id_embedding_dict: Dictionary containing item IDs and their embeddings
        """
        ...
        self.tree = BallTree(self.normalized_embeddings, metric='cosine')

    def find_similar(self, query_embedding: np.ndarray, top_k: int = 5) -> dict:
        """
        Find top-k most similar items to the query
        :param query_embedding: Raw embedding vector of the query
        :param top_k: Number of results to return
        :return: Sorted dictionary {item_id: similarity_score}
        """
